...
impl ResizeMlp {
    fn init(
        ...
        Self {
            linear_fc1: crate::linear_config(input_size, intermediate_size).init(device),
            linear_fc2: crate::linear_config(intermediate_size, output_size).init(device),
            act,
        }
    }
...
impl CodePredictor {
    fn init(cfg: &CodePredictorConfig, talker_hidden_size: usize, device: &Device) -> Self {
        ...
        Self {
            ...
            lm_head: (0..n)
                .map(|_| {
                    crate::linear_config(cfg.hidden_size, cfg.vocab_size)
                        .with_bias(false)
                        .init(device)
                })
                .collect(),
            small_to_mtp_projection: (cfg.hidden_size != talker_hidden_size)
                .then(|| crate::linear_config(talker_hidden_size, cfg.hidden_size).init(device)),
        }
    }
...
impl Model {
    fn init(cfg: &Config, device: &Device) -> Self {
        ...
        let talker = Talker {
            ...
            codec_head: crate::linear_config(tc.hidden_size, tc.vocab_size)
                .with_bias(false)
                .init(device),
