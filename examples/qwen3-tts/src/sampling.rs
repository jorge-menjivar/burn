...
impl Sampling {
    ...
    ///
    /// One kernel does it on the GPU backends, see [`crate::sampling_kernel`]; elsewhere it is
    ...
    pub fn draw(&self, logits: Tensor<1>, noise: Tensor<1>) -> Tensor<1, Int> {
        ...
        if KERNEL.load(Ordering::Relaxed) && crate::sampling_kernel::available(&logits.device()) {
            ...
            return crate::sampling_kernel::draw_token(
                logits,
                noise,
                temperature,
                top_k,
...
mod tests {
    ...
    fn kernel_matches_operations() {
        let device = device();
        if !crate::sampling_kernel::available(&device) {
            return;
        }
        device.seed(7);
        let cases = [
